...
class SandDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        particle_type = torch.from_numpy(data['particle_type']).int() - 5
        particle_num = particle_type.shape[0]
        particle_r = torch.ones(particle_num)
        particle_m = torch.ones(particle_num)
        nonk_mask = torch.from_numpy(nonk_mask.astype(np.int32))
        tgt_poss = torch.from_numpy(tgt_poss.astype(np.float32))

        return poss, vels, tgt_accs, tgt_vels, particle_r, particle_type, nonk_mask, tgt_poss, idx, particle_m
